...
<div align="center">
<img name="square" src="./images/rectangle.png" width="350"/>
</div>
...
圆实际是以其内切正多边形来近似表示的，其边的数量由`steps`指定。如果未指定边数则会自动确定为一个弧度为`extent`的圆弧。在指定了`steps`之后，海龟会将该圆弧内切成`steps`条连接的行长线段，如果该圆弧为一个完整的圆，那么绘制出来的则为一个正`steps`边形。
